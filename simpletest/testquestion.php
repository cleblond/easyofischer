...
/**
 * Unit tests for the easyofischer question definition class.
 *
 * @package    qtype
...
require_once($CFG->dirroot . '/question/type/easyofischer/simpletest/helper.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_easyofischer_question_test extends UnitTestCase {
    public function test_compare_string_with_wildcard() {
        // Test case sensitive literal matches.
        $this->assertTrue(qtype_easyofischer_question::compare_string_with_wildcard(
                'Frog', 'Frog', false));
        $this->assertFalse(qtype_easyofischer_question::compare_string_with_wildcard(
                'Frog', 'frog', false));
        $this->assertTrue(qtype_easyofischer_question::compare_string_with_wildcard(
                '   Frog   ', 'Frog', false));
        $this->assertFalse(qtype_easyofischer_question::compare_string_with_wildcard(
    ...
    public function test_is_complete_response() {
		$question = qtype_easyofischer_test_helper::make_a_easyofischer_question();
    ...
    public function test_get_question_summary() {
        $easyofischer = qtype_easyofischer_test_helper::make_a_easyofischer_question();
        $qsummary = $easyofischer->get_question_summary();
        $this->assertEqual('Name an amphibian: __________', $qsummary);
    }

    public function test_summarise_response() {
        $easyofischer = qtype_easyofischer_test_helper::make_a_easyofischer_question();
        $summary = $easyofischer->summarise_response(array('answer' => 'dog'));
        $this->assertEqual('dog', $summary);
    }

    public function test_classify_response() {
        $easyofischer = qtype_easyofischer_test_helper::make_a_easyofischer_question();
        $easyofischer->start_attempt(new question_attempt_step(), 1);

        $this->assertEqual(array(
                new question_classified_response(13, 'frog', 1.0)),
                $easyofischer->classify_response(array('answer' => 'frog')));
        $this->assertEqual(array(
                new question_classified_response(14, 'toad', 0.8)),
                $easyofischer->classify_response(array('answer' => 'toad')));
        $this->assertEqual(array(
                new question_classified_response(15, 'cat', 0.0)),
                $easyofischer->classify_response(array('answer' => 'cat')));
        $this->assertEqual(array(
                question_classified_response::no_response()),
                $easyofischer->classify_response(array('answer' => '')));
    }
}
